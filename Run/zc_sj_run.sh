...
do
        ...
        m=$(date "+%M")
        now=`expr $h + $m`
        if [ 569 -lt $now -a $now -lt 571 ] || [ 1049 -lt $now -a $now -lt 1051 ]
        then
                ...
                do
                        ...
                        if [ $count -eq 0 ]
                        then
                                echo ${time}    $name start process.....  >> /home/zyl/NewsSearch/Logs/zc_sj_run.log
                                nohup /usr/local/python3/bin/python3  /home/zyl/NewsSearch/Quotation/$name >/dev/null 2>&1 &
                        else
                                echo ${time}    $name runing.....  >> /home/zyl/NewsSearch/Logs/zc_sj_run.log
                        fi
                done
        fi
done